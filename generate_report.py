...
def format_project(project):
    ...
    cm_tasks = get_tasks(cm_section["gid"])
    cm_details = [get_task_details(t["gid"]) for t in cm_tasks]
    incomplete = [t for t in cm_details if is_incomplete(t)]
    if not incomplete:
        return None
    ...
    launch_milestone = next((t for t in cm_details if "launch" in t["name"].lower()), None)
    launch_date = None
    if launch_milestone and not launch_milestone.get("completed"):
        launch_date = launch_milestone.get("due_on")

    all_comments = []
    for s in sections:
        tasks = get_tasks(s["gid"])
        for t in tasks:
    ...
    top_comments = [f"- {c[1]}" for c in all_comments[:6]]

    return {
        "name": project["name"],
        "next": f'{next_milestone["name"]} – {next_milestone.get("due_on", "No date")}',
        "launch": launch_date if launch_date else "None",
